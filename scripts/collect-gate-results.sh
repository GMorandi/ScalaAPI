...
GitHub Actions API and verify that every expected paired-CI gate is present and
...
# Every expected paired-CI gate by its check-run name. Keep this list in sync
# with the job names in .github/workflows/ci.yml.
expected_gates=(
    "Immutable pair and contract"
    "Gateway build, test, and benchmark smoke"
    "Platform greenfield, tests, and benchmark smoke"
    "admin-web typecheck, build, and e2e"
    "user-web typecheck, build, and e2e"
    "Build and scan gateway image"
    "Build and scan platform image"
    ...
    "Publish validated pair manifest"
)
...
for gate_name in "${expected_gates[@]}"; do
    matching_jobs="$(jq --arg name "$gate_name" \
        'map(select(.name == $name))' <<<"$jobs_json")"
    ...
    job_url="$(jq -r --arg name "$gate_name" \
        'map(select(.name == $name))[0].html_url' <<<"$jobs_json")"
...
if (( ${#missing_gates[@]} > 0 || ${#failed_gates[@]} > 0 )); then
    fail "the producing run does not demonstrate a fully green paired CI"
fi
