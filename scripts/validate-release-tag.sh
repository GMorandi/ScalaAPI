...
tag_sha="$(git -C "$repo_root" rev-parse "refs/tags/$version^{commit}" 2>/dev/null)" ||
    fail "tag does not exist in the superproject: $version"

[[ "$tag_sha" == "$head_sha" ]] ||
    fail "superproject tag $version resolves to $tag_sha, not checked-out $head_sha"
...
echo "release tag $version identifies superproject commit $head_sha"
